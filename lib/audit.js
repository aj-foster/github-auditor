...
exports.queryUsers = async function () {
  ...
  let users = res.organization.members.nodes
  let hasNextPage = res.organization.members.pageInfo.hasNextPage

  while (hasNextPage) {
    let cursor = res.organization.members.pageInfo.endCursor
    ...
    users = users.concat(res.organization.members.nodes)
    hasNextPage = res.organization.members.pageInfo.hasNextPage
  }
